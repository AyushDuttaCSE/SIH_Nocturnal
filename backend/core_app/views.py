...
try:
    from .ml_engine import generate_full_ml_evaluation
except ImportError:
    generate_full_ml_evaluation = None
...
def generate_feasibility(request):
    ...
    try:
        ...
        ml_output = None
        if generate_full_ml_evaluation:
            try:
                ml_output = generate_full_ml_evaluation(
        ...
        if ml_output:
            ml_viability = ml_output["ml_result"]["enterprise_viability"]
            ml_risk_index = ml_output["ml_result"]["risk_index"]
            ...
            bank_dpr_summary = (
                ...
                f"Risk Index of {ml_risk_index}/100. "
                f"Decision: {ml_recommendation}. "
                f"{ml_rationale}"
            )
        else:
            ml_viability, ml_risk_index, ml_confidence, ml_recommendation, bank_dpr_summary = None, None, None, None, None
        ...
        if isinstance(report, dict):
            report.update({
                ...
                "ml_risk_index": ml_risk_index,
                "ml_recommendation": ml_recommendation,
                "bank_dpr_summary": bank_dpr_summary
            })
